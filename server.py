    ...
    def handle_request(self, method):
        ...
        if handler:
                    ...
                    if "multipart/form-data" in content_type:
                        ...
                        try:
                            multipart_fields = parse_multipart_form(
                                self.rfile, self.headers, content_length
                            )
            ...
            try:
                if is_cms_request:
                    response = handler(
                        path, simple_params, form_data, self, csrf_token=csrf_token
                    )
                else:
        ...
        formatted_stories.append(
            {
                ...
                "published_at": format_date(story.get("published_at"))
                or format_date(story.get("created_at")),
            }
        )
...
def cms_stories_handler(path, params, form_data, handler, csrf_token=None):
    ...
    for story in stories:
        ...
        status_badge_class = (
            "status-badge published" if is_published else "status-badge draft"
        )
    ...
    context = {
        ...
        "has_filters": bool(
            search_query or category_filter or status_filter or month_filter
        ),
...
def cms_edit_handler(path, params, form_data, handler, csrf_token=None):
    ...
    if form_data and "title" in form_data:
        # Process form submission
        # Handle file upload for featured_image
        featured_image_path = form_data.get(
            "featured_image", story.get("featured_image", "")
        )
